...
impl Prover for BoundlessProver {
    ...
    async fn shasta_aggregate(
            ...
            .map(|p| {
                p.input.ok_or_else(|| {
                    ProverError::GuestError(
                        "Missing input in proof for shasta aggregation".to_string(),
                    )
                })
            })
            .collect::<ProverResult<Vec<_>>>()?;
        ...
        let agent_input = ShastaRisc0AggregationGuestInput {
        ...
        let payload = serde_json::json!({
            "input": agent_input_bytes,
            "proof_type": "Aggregate",
            // Receipts are needed as assumptions for env::verify calls in the guest.
