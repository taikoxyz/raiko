...
use crate::prover::{
    ...
    request::{Risc0ProofParams, Risc0Response},
};
...
pub async fn execute_risc0(
    ...
    let result =
        maybe_prove::<Output>(req, &input, RISC0_GUEST_ELF, &output, Default::default()).await;
...
pub async fn maybe_prove<O: Eq + Debug + Serialize + DeserializeOwned>(
    req: &Risc0ProofParams,
    input: &Risc0Input,
    elf: &[u8],
    expected_output: &O,
