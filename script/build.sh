...
if [ -z "$1" ] || [ "$1" == "risc0" ]; then
	...
	if [ -z "${RUN}" ]; then
		...
		if [ -z "${TEST}" ]; then
			...
			cargo ${TOOLCHAIN_SP1} test ${FLAGS} --lib risc0-driver --features risc0 -- run_risc0_test_elf 
...
if [ -z "$1" ] || [ "$1" == "sp1" ]; then
	...
	if [ -z "${RUN}" ]; then
		...
		if [ -z "${TEST}" ]; then
			...
			echo "Running Sp1 tests"
			cargo ${TOOLCHAIN_SP1} test ${FLAGS} --lib sp1-driver --features sp1 -- run_sp1_test_elf 
