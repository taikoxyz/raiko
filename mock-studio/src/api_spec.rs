...
pub fn shasta_api_spec() -> ShastaApiSpec {
    ShastaApiSpec {
        ...
        key_request_fields: vec![
            ...
            "proof_type: string; existing mock helpers preserve it in the response envelope"
                .to_string(),
        ...
        response_contract: vec![
            ...
            "Use mock_proof_response(body, label) for proof-shaped success responses".to_string(),
        ...
        helper_contract: vec![
            ...
            "Required imports: use serde_json::Value; and use crate::{error_status, mock_proof_response, ok_status, proof_type, proposal_batch_id, MockContext};".to_string(),
        ...
        reference_snippets: vec![
            ...
            "Example stateful pattern: if aggregate is true return error_status(...); else if ctx.has_seen_request(body) return mock_proof_response(body, label); else ctx.mark_request_seen(body) and return ok_status(..., \"registered\")".to_string(),
        ],
    }
}
