    ...
    pub fn success() -> Self {
        Self {
            spec: MockSpec {
                ...
                normal_request_policy: None,
                aggregation_policy: None,
                proof_response_policy: None,
...
impl MockPlanner for OpenRouterAgent {
    async fn plan(&self, requirement: &str) -> anyhow::Result<SpecGeneration> {
        ...
        let content = self
            .call_json(
                &self.spec_model,
                "You are planning a Rust mock gateway. Return only JSON with keys summary, default_task_status, memory_required, request_key_fields, normal_request_policy, aggregation_policy, proof_response_policy, nth_responses. Keep the result constrained to /v3/proof/batch/shasta behavior.",
                &prompt,
            )
            .await?;
...
impl RestrictedHandlerGenerator for OpenRouterAgent {
    async fn generate_handler(
        ...
        validate_handler_source(source)?;
        Ok(HandlerGeneration {
...
pub fn render_handler_from_spec(spec: &MockSpec) -> String {
    ...
    format!(
        "use serde_json::Value;\n\nuse crate::{{error_status, mock_proof_response, ok_status, proof_type, proposal_batch_id, MockContext}};\n\npub fn handle_shasta_request(ctx: &MockContext, body: &Value) -> Value {{\n    match ctx.call_index() {{\n{match_arms}        _ => ok_status(proof_type(body), proposal_batch_id(body), {:?}),\n    }}\n}}\n",
...
fn validate_handler_source(source: &str) -> anyhow::Result<()> {
    ...
    if !has_valid_error && !has_valid_ok && !has_valid_mock_proof {
...
fn build_planner_prompt(requirement: &str, api_spec: &ShastaApiSpec) -> anyhow::Result<String> {
    Ok(format!(
        "Requirement:\n{requirement}\n\nAPI Spec:\n{}\n\nReturn JSON only. Required keys: summary, default_task_status, memory_required, request_key_fields, normal_request_policy, aggregation_policy, proof_response_policy, nth_responses. nth_responses is an array of objects with keys n, kind, error, message. default_task_status must be a snake_case task status string.",
...
mod tests {
    ...
    use super::{
        build_handler_prompt, extract_openrouter_message, parse_mock_spec, validate_handler_source,
    };
