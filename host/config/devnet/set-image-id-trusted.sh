...
docker run --rm \
  ...
  -e BATCH_PROOF_IMAGE="$GUEST_PROOF_IMAGE" \
  ...
  $IMAGE sh -c '
    ...
    cast send $RISC0_VERIFIER_ADDRESS "setImageIdTrusted(bytes32,bool)" $BATCH_PROOF_IMAGE true --rpc-url $FORK_URL --private-key $PRIVATE_KEY
  '